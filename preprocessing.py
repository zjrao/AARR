...
def extract_features(config):
    ...
    sptype = h5py.special_dtype(vlen=str)
    all_features = np.array([a.encode('utf8') for a in all_features])
    print("-----------------", all_features[0].dtype)
    print("-----------------", all_features.dtype)
    print("---------------", att.dtype)
    f.create_dataset('feature_map', data=all_features,compression=compression, dtype=sptype)
