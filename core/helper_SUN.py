...
def val_gzsl(dataloader, in_package, bias=0):
    model = in_package['model']
    device = in_package['device']
    bs = in_package['batch_size']
    target_classes = dataloader.dataset.seenclasses
    test_label = dataloader.dataset.data['test_seen']['labels']
...
def val_zs_gzsl(dataloader, in_package, bias=0):
    batch_size = in_package['batch_size']
    model = in_package['model']
    device = in_package['device']
    unseen_classes = dataloader.dataset.unseenclasses
    test_label = dataloader.dataset.data['test_unseen']['labels']
    with torch.no_grad():
        ...
        for iter, (img, label) in enumerate(dataloader):
            ...
            end = iter * batch_size + img.size(0)
...
def eval_zs_gzsl(dataloader_seen, dataloader_unseen, model, device, bias_seen, bias_unseen):
    model.eval()
    batch_size = 256
    in_package = {'model': model, 'device': device, 'batch_size': batch_size}

    with torch.no_grad():
        acc_seen = val_gzsl(dataloader_seen, in_package, bias=bias_seen)
        acc_novel, acc_zs = val_zs_gzsl(dataloader_unseen, in_package, bias=bias_unseen)
