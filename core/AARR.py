...
class AARR(nn.Module):
    ...
    def __init__(self, dim_f, dim_v,
                 ...
                 is_sigmoid=False, lock_lowlevel_network=False, isFT=True):
        super(AARR, self).__init__()
        self.dim_f = dim_f
        self.dim_v = dim_v
        ...
        if init_w2v_att is None:
            self.V = nn.Parameter(nn.init.normal_(torch.empty(self.dim_att, self.dim_v)), requires_grad=trainable)
        else:
        ...
        self.W_1 = nn.Parameter(nn.init.normal_(torch.empty(self.dim_v, self.dim_f)),
                                requires_grad=trainable)  # nn.utils.weight_norm(nn.Linear(self.dim_v,self.dim_f,bias=False))#
        self.W_2 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v, self.dim_f)),
                                requires_grad=trainable)  # nn.utils.weight_norm(nn.Linear(self.dim_v,self.dim_f,bias=False))#
        ## second layer attenion conditioned on image features
        self.W_3 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v, self.dim_f)), requires_grad=trainable)

        self.W_1_1 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_f, self.dim_v)),
                                  requires_grad=trainable)  # nn.utils.weight_norm(nn.Linear(self.dim_v,self.dim_f,bias=False))#
        self.W_2_1 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_v, self.dim_f)), requires_grad=trainable)
        self.W_3_1 = nn.Parameter(nn.init.zeros_(torch.empty(self.dim_f, self.dim_v)), requires_grad=trainable)
        ...
        if desired_mass == -1:
            self.desired_mass = self.unseenclass.size(
                0) / self.nclass  # nn.Parameter(torch.tensor(self.unseenclass.size(0)/self.nclass),requires_grad = False)#nn.Parameter(torch.tensor(0.1),requires_grad = False)#
        else:
        ...
        # bakcbone
        self.lock = lock_lowlevel_network
        resnet101 = torchvision.models.resnet101(pretrained=True)
        resnet101_2 = torchvision.models.resnet101(pretrained=True)
        self.bnet = nn.Sequential(*list(resnet101.children())[0:4])#4
        if self.lock:
            self.onet = nn.Sequential(*list(resnet101.children())[4:-2])#4
            self.mnet = nn.Sequential(*list(resnet101_2.children())[4:-2])#4
        else:
            self.onet = nn.Sequential(*list(resnet101.children())[0:-2])  # 4
            self.mnet = nn.Sequential(*list(resnet101_2.children())[0:-2])  # 4

        for name, param in self.bnet.named_parameters():
            param.requires_grad = False
        for name, param in self.onet.named_parameters():
            param.requires_grad = False
        for name, param in self.mnet.named_parameters():
            param.requires_grad = isFT

        #self.classifier1 = nn.Linear(2048, 717)
    ...
    def forward(self, Fs):
        ...
        #Fs_cls = F.avg_pool2d(Fs, (Fs.size(2), Fs.size(3))).view(Fs.size(0), -1)
        #cls_output = self.classifier1(Fs_cls)
        cls_output = torch.tensor([0.0])
        ...
        if self.uniform_att_1:  # false
            S_p = torch.einsum('bir,bir->bi', A_b,
                               S)  # ablation: compute attribute score using average image region features
        else:
        ...
        if self.uniform_att_2:  # true
            S_pp = torch.einsum('ki,bi,bi->bik', self.att, A_b_p,
                                S_p)  # ablation: setting attention over attribute to 1
        else:
        ...
        package1 = {'S_pp': S_pp, 'Pred_att': Pred_att, 'S_b_pp': S_b_pp, 'A_p': A_p, 'A': A, 'S_attr': S_attr,
                    'visualf_ori': visualf_ori, 'a_v': F_p, 'Fs_o':Fs_o, 'Fs_m':Fs_m, 'out_cls':cls_output}
        ...
        S_pp = torch.einsum('ki,bi->bik', self.att,
                            S_p)  # compute the final prediction as the product of semantic scores, attribute scores, and attention over attribute scores
