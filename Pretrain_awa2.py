...
model = AARR(dim_f, dim_v, init_w2v_att, att, normalize_att,
             ...
             is_bias=True, lock_lowlevel_network=True, isFT=False)
model.to(device)
...
for i in range(0, nepoches):
    ...
    if H > best_performance[2]:
        ...
        torch.save(model.state_dict(), './model/pretrained_awa2_h.pth')
    if acc_zs > best_acc:
        best_acc = acc_zs
        torch.save(model.state_dict(), './model/pretrained_awa2_z.pth')
    print('epoch=%d, acc_unseen=%.3f, acc_seen=%.3f, H=%.3f, acc_zs=%.3f'\
