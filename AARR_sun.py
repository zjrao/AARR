...
model.load_state_dict(torch.load('./model/pretrained_sun_h.pth'), strict=False)
gradmodel = Gradmap(label_map, 0.1, mode=2, n=717, attsize=102, uniform_att_2=model.uniform_att_2).to(device)
gradmodel.load_state_dict(torch.load('./model/pretrained_sun_h.pth'), strict=False)

protoNet = computeProto(num_classes=717, num_att=102, dim_f=dim_f, num_sample=len(dataset_loader),
...
for i in range(0, nepoches):
    ...
    for iter, (img, label, att) in enumerate(dataset_loader):
        ...
        loss= loss_b + 15*loss_align + 0.1*proto_loss
    ...
    if H > best_performance[2]:
        ...
        torch.save(model.state_dict(), './model/best_H_SUN.pth')
    if acc_zs > best_acc:
        best_acc = acc_zs
        torch.save(model.state_dict(), './model/best_Z_SUN.pth')
    print('epoch=%d, acc_unseen=%.3f, acc_seen=%.3f, H=%.3f, acc_zs=%.3f'\
